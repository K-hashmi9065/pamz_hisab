...
/// Form/Modal sheet to record funds returned back to a contact.
class FLReturnForm extends ConsumerStatefulWidget {
  const FLReturnForm({
    super.key,
  ...
  @override
  ConsumerState<FLReturnForm> createState() => _FLReturnFormState();
}

class _FLReturnFormState extends ConsumerState<FLReturnForm> {
  ...
  Widget build(BuildContext context) {
    ...
    final summaryAsync = _selectedContactId != null
        ? ref.watch(flContactSummaryProvider(_selectedContactId!))
        : null;
    final availableAmount = summaryAsync?.valueOrNull?.availableAmount;
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintText: 'Reason for return or remarks...',
                      ),
                      maxLines: 2,
                    ),
                    SizedBox(height: AppSpacing.md.h),
  ...
  Future<void> _submit() async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Failed to record fund return'),
          backgroundColor: AppColors.error,
        ),
      );
    }
